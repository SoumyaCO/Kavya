## TODO

- [x] Add the database and database configurations
- [ ] Add Article section.
- [x] Add a dropdown in writing page.
- [ ] Add a Contact section.
- [ ] Uploading photo in Blogs should be optional.
- [ ] Home page slider with best articles, blogs, and poems.
- [ ] After the featured articles, blogs and poems, the blogs will be started there, not required the other sections.
- [ ] The blogs section will be at in the HOME page, not required to make another blogs page (can make that into article page).
- [ ] Add social links addition screen.
- [ ] The accounts can be deleted. and also after 30 days can be recovered.

**NOTE:** Articles/Blogs section should be the main priority
